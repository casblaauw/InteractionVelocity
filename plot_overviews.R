...
# --- Histogram of the different cell type counts
...
# --- Barplot of marker presence in classes
...
marker_pct <- all_markers %>%
  ...
  mutate(ID = tidytext::reorder_within(ID, diff, types)) %>% # Sort within facets (needs tidytext::scale_x_reordered)
  # Make base plot
  ggplot() + 
